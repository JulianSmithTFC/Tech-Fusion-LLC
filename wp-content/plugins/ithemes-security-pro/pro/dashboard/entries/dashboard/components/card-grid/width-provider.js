 ...
 */
import { BREAKPOINTS, BREAKPOINT_ORDER } from '../../utils';
	...
	return class WidthProvider extends Component {
		...
		onWindowResize = () => {
			...
			if ( node instanceof window.HTMLElement ) {
				...
				if ( this.props.onWidthBreakpoint ) {
					for ( const bp of BREAKPOINT_ORDER ) {
						if ( BREAKPOINTS.hasOwnProperty( bp ) && width < BREAKPOINTS[ bp ] ) {
							this.props.onWidthBreakpoint( bp );
							return;
						}
					}

					this.props.onWidthBreakpoint( BREAKPOINT_ORDER[ 0 ] );
				}
			}
		};
