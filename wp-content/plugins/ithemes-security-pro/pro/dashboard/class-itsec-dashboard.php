...
class ITSEC_Dashboard {
	...
	public function enqueue( $hook ) {
		...
		if ( defined( 'ITSEC_DEBUG' ) && ITSEC_DEBUG && file_exists( dirname( __FILE__ ) . '/js/dashboard.js' ) ) {
			$js_file = 'dashboard.js';
		} else {
			$js_file = 'dashboard.min.js';
		}

		if ( defined( 'ITSEC_DEBUG' ) && ITSEC_DEBUG && file_exists( dirname( __FILE__ ) . '/css/dashboard.css' ) ) {
			$css_file = 'dashboard.css';
		} else {
			$css_file = 'dashboard.min.css';
		}

		wp_enqueue_script( 'itsec-security-dashboard', plugins_url( "js/{$js_file}", __FILE__ ), array(
			'wp-a11y',
			'wp-api-fetch',
			'wp-components',
			'wp-compose',
			...
			'lodash',
			'react',
		), ITSEC_Core::get_plugin_build(), true );
		wp_enqueue_style( 'itsec-security-dashboard', plugins_url( "css/{$css_file}", __FILE__ ), array(
			'wp-components'
		), ITSEC_Core::get_plugin_build() );

		$uid = get_current_user_id();

		if ( ( ! $primary_id = (int) get_user_meta( $uid, self::META_PRIMARY, true ) ) || self::CPT_DASHBOARD !== get_post_type( $primary_id ) ) {
			require_once( dirname( __FILE__ ) . '/class-itsec-dashboard-util.php' );

			if ( $owned = ITSEC_Dashboard_Util::get_owned_dashboards() ) {
				$primary_id = (int) $owned[0]->ID;
			} elseif ( $shared = ITSEC_Dashboard_Util::get_shared_dashboards() ) {
			...
			if ( $primary_id ) {
				update_user_meta( $uid, self::META_PRIMARY, $primary_id );
			}
		}
		...
		foreach ( $preload_requests as $key => $config ) {
			if ( is_string( $config ) ) {
				$key    = $config;
		...
		wp_localize_script( 'itsec-security-dashboard', 'iThemesSecurityDashboard', array(
			'rootURL'           => rest_url(),
			...
			'logs_url'          => ITSEC_Core::get_logs_page_url( 'malware' ),
			'roles'             => $roles,
			'preload'           => $preload,
